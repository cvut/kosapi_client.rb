  ...
  describe '.parse' do
    ...
    context 'with no links' do
      it 'returns instance with no links set' do
        links = described_class.parse(nil, client: client)
    ...
    context 'with both links' do
      it 'parses both links' do
        links = described_class.parse([{rel: 'prev', href: 'courses/?offset=0&limit=10'}, {rel: 'next', href: 'courses/?offset=20&limit=10'}], client: client)
    ...
    context 'with next link' do
      it 'parses next link' do
        links = described_class.parse({rel: 'next', href: 'courses/?offset=20&limit=10'}, client: client)
        expect(links.next.link_href).to eq 'courses/?offset=20&limit=10'
    ...
    context 'with prev link' do
      it 'parses prev link' do
        links = described_class.parse({rel: 'prev', href: 'courses/?offset=20&limit=10'}, client: client)
        expect(links.next).not_to be
